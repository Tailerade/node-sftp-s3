        ...
        client
          .on('authentication', (ctx) => {
            if (ctx.method !== 'publickey') {
              this._log('rejecting non-public-key authentication: ' + ctx.method);
                  ...
                  .on('READDIR', (reqid, handle) => {
                    ...
                    sftpStream.name(
                      ...
                      state.listings.map((l) => {
                        const filename = l.Key.substring(state.fullname.length);
  ...
  _findKey(username, key) {
    for (let idx = 0; idx < this.publicKeys.length; idx++) {
      if (this.publicKeys[idx]['username'] == username) {
        ...
        if (key.algo === pubKey.key.type && buffersEqual(key.data, pubKey.key.getPublicSSH())) {
          return pubKey;
        }
      }
    }
